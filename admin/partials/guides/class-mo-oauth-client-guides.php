	...
	public static function instructions($appname) {
	 ...
	 public static function instructions_page($appname){
		...
		if(strpos($app->label, 'Custom') !== false) {
			echo '<br><strong>Instructions to configure '. esc_attr( $app->label ).':</strong><ol>';
		} else {
			echo '<br><strong>Instructions to configure '.esc_attr( $app->label ).' as OAuth Server:</strong><ol>';
		}
		echo '<li>In front of <b>App Name</b> field, enter the name you would like to display on login button.</li>';
		if($app->guide !== "") {
			echo '<li>Please follow the instruction given in Step by Step guide to get your Client ID, Client Secret and other necessary information. <a href="'.esc_url( $app->guide ).'" target="_blank">Click here</a> to get the step by step guide to configure <i>'.esc_attr( explode('App', $app->label)[0] ).'</i> as OAuth/OpenID Connect server.</li>';
		} else {
			echo '
