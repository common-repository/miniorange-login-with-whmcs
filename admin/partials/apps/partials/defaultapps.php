	...
	<ul id="mo_oauth_client_default_apps">
		...
		<?php
			...
			foreach($defaultappsjson as $appId => $application) {
				...
				echo '<li data-appid="'.esc_attr( $appId ).'"><a ' . ( 'cognito' === $appId ? 'id=vip-default-app' : '' ) . ' href="#"><img class="mo_oauth_client_default_app_icon" src="'. esc_url( plugins_url( '../images/'. $application->image , __FILE__ ) ).'"><br>'.esc_attr( $application->label ).'</a></li>';
			}
		?>
		</div>
		...
		<div id="mo_oauth_client_custom_apps">
			<?php
				foreach( $custom_apps as $appId => $application ) {
					echo '<li data-appid="'.esc_attr( $appId ).'"><a href="#"><img class="mo_oauth_client_default_app_icon" src="'. esc_url( plugins_url( '../images/'. $application->image , __FILE__ ) ).'"><br>'.esc_attr( $application->label ).'</a></li>';
				}
			?>
		</div>
	</ul>
